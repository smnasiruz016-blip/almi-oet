...
describe("the state that is CORRECT — recorded, after the fix", () => {
  // Measured on production in post-deploy run 34428615639, commit 908b5aa.
  // Two populations, both predictable from the mechanism: build-time pages hit on
  // the 2nd request, on-demand pages on the 3rd.
  const AFTER: [string, string[], number][] = [
    ["/", ["PRERENDER", "HIT", "HIT"], 2],
    ["/nursing", ["PRERENDER", "HIT", "HIT"], 2],
    ["/nursing/from-india", ["MISS", "MISS", "HIT"], 3],
    ["/nursing/from-india/uk-nmc", ["MISS", "MISS", "HIT"], 3],
    ["/register/uk-nmc", ["MISS", "MISS", "HIT"], 3],
  ];
