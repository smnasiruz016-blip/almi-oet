...
// The caveat is PRINTED, not left in a comment: the number is meaningless off a
// cold cache, and the next person to run this by hand will otherwise read a
// column of 1s as good news.
console.log("  VALID ONLY ON A POST-DEPLOY RUN, against the cold cache a deploy just emptied.");
console.log("  A manual re-run hits a WARM cache: every route reports 1, and that is NOT its warm-up.");
