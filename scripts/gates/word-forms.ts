...
/**
 * 🔴 THE MARKER JOINS WHERE THIS CHECK SPLITS, AND THE MARKER WINS.
 *
 * normalize() joins tokens with no separator, so a script that says "twenty
 * nineteen" carries the answer 2019 — 20 and 19 become 2019 on both sides, and
 * the MARKER accepts it. Asking token by token cannot see that, and two
 * components disagreeing about whether an answer is present is the defect these
 * checks exist to catch, not one to introduce. gate:length's findability check
 * was corrected the same way on 7 September.
 *
 * ⚠️ IT SPANS TOKENS, IT DOES NOT WANDER INSIDE THEM. The match must still
 * begin where a source token begins and end where one ends — runOnMatch, per
 * word. Measured over the whole bank BEFORE it was applied, because a fallback
 * is a hole until it is counted:
 *
 *     unaligned containment   rescues 2 rows, one of them `lab` for "a
 *                             labrador", which is not a number at all — and
 *                             quietly disarms EIGHT written exemption rows:
 *                             hep B, C. diff, OT, 40 km, micromol per litre,
 *                             urinary tract. It reaches a zero by undoing the
 *                             owner's own rulings.
 *     boundary-aligned        rescues ONE row, "in 2019", and disarms nothing.
 *
 * So the aligned form is the one that went in. If it ever rescues something
 * that is not a number or a notation, it has become the unaligned rule wearing
 * a better name, and the red is worth more.
 */
export function wordFoundInSource(
  word: string,
  sourceWords: Set<string>,
  joined: string,
  tokens: string[],
): boolean {
  if (matchInSource(word, sourceWords) !== null) return true;
  return runOnMatch(joined, tokens, word);
}
