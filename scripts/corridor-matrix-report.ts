...
import { corridorsFor } from "../src/lib/oet-seo/corridor-compose";
import { UK_DELTAS, AU_DELTAS } from "../src/lib/oet-seo/destination-deltas";

// The Australia corridors are built HERE, on demand, and are not wired into the
// emitted set — the grid they belong to was retired on 2026-08-06. This report
// is the evidence for that decision, so it stays runnable: anyone can re-derive
// the same-origin cross-destination numbers without the grid existing in
// production. Re-measure before reopening the question.
const AU_CORRIDORS = corridorsFor(AU_DELTAS);
