...
import { regulatorProfileForOrg } from "@/lib/oet-seo/regulator-profiles";
...
export function RegisterOrgPage({ orgSlug }: { orgSlug: string }) {
  ...
  const sharedFaqs = regulatorProfileForOrg(orgSlug)?.faq ?? destinationForOrg(orgSlug)?.sharedFaq ?? [];
